...
class LukeLinkGlobalConfidence(LukeModel):
    def __init__(
            self,
    ...
    def forward(self, batch: ModelExample):
        ...
        while entity_mask.sum() > 0:
            ...
            for key, group in groupby(seq_index_2_coordinate, key=lambda co: co[0]):
                context_list_coordinate = list(group)
                context_list_seq_index = [coordinate_2_seq_index[coo] for coo in context_list_coordinate]
                context_top1_scores = topk_scores[:, 0][context_list_seq_index]  # [context_mask_entity_num]
                context_argmax_coordinate = context_list_coordinate[context_top1_scores.argmax(dim=0)]
                list_current_predict_coordinate.append(context_argmax_coordinate)
